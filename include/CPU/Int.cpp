...
void Int::SetBaseN(int base, const char* digits, const char* value) {
  CLEAR();

  Int pw((uint64_t)1);
  Int nb((uint64_t)base);
  Int c;

  int lgth = (int)strlen(value);
  for (int i = lgth - 1; i >= 0; i--) {
    const char* p = strchr(digits, toupper(value[i]));
    if (!p) {
      printf("Invalid character in input value!\n");
      return;
    }
    int id = (int)(p - digits);
    c.SetInt32(id);
